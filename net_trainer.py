...
class RPNTrainer:
    ...
    def trainOne(self,batch_size,learning_rate,rounds = 100):
        """
        batch_size:图片数
        learning_rate:学习率
        rounds:运算次数
        ...
        weights = {
            ...
            'feature':tf.compat.v1.get_variable(name = 'w_feature',shape = [1,1,1024,2])
            }
        biases = {
            'down':tf.compat.v1.get_variable(name = 'b_down',shape = [1024,]), # 降采样
            'feature':tf.compat.v1.get_variable(name = 'b_feature',shape = [2,])
        }
        ...
        for index in range(len(img_batch)):
            ...
            # 训练bounding_box
            bbox_accuracy = trainBbox(img,bbox,label,select)
...
if __name__ == '__main__':
    trainer = RPNTrainer()
    trainer.trainOne(batch_size = 3,learning_rate = 0.0001)